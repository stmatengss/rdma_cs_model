...
out:
		for (i = 0; i < connections; i++) {
				...
				if (ret) printf("mckey: failure leaving: %d\n", ret);
		}
		return ret;
}
...
int main(int argc, char **argv) {
		...
		while ((op = getopt(argc, argv, "m:M:sb:c:C:S:p:")) != -1) {
				...
				switch (op) {
						...
						case 'm': dst_addr = optarg; break;
						case 'M':
								  unmapped_addr = 1;
								  dst_addr = optarg;
								  break;
						case 's': is_sender = 1; break;
						case 'b':
								  src_addr = optarg;
								  test.src_addr = (struct sockaddr *)&test.src_in;
								  break;
						case 'c': connections = atoi(optarg); break;
						case 'C': message_count = atoi(optarg); break;
						case 'S': message_size = atoi(optarg); break;
						case 'p': port_space = strtol(optarg, NULL, 0); break;
						default:
								  printf("usage: %s\n", argv[0]);
